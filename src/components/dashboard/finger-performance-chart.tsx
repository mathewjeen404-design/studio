...
export function FingerPerformanceChart() {
  const { stats } = useTypingStats();
  
  const chartData = FINGERS.map(finger => ({
    name: finger.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()),
    wpm: stats.fingerStats[finger].wpm || 0,
    errorRate: (stats.fingerStats[finger].errorRate || 0) * 100,
  }));
